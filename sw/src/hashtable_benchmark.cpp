...
void fillHashTable(unsigned char* block, uint64_t remoteBaseAddr, uint64_t numElements, uint64_t valueSize) {
   ...
   for (uint32_t e = 0; e < numElements; ++e) {
      ...
      //std::cout << std::dec << e << ": " << std::hex << ePtr->key << " -> " << ePtr->value_ptr << std::endl;
      ePtr++;
...
void printHashTable(unsigned char* block, uint64_t numElements, uint64_t valueSize) {
...
int main(int argc, char *argv[]) {
   ...
   uint64_t hashTableSize = numberOfElements * (sizeof(HashTableEntry) + valueSize);
   uint64_t resultSize = numberOfQueries * (sizeof(HashTableEntry) + valueSize);
   uint64_t allocSize = hashTableSize + resultSize;
   ...
   if (nodeId == 1) {
      ...
      fillHashTable((unsigned char*) dmaBuffer, (uint64_t) remoteBaseAddr, numberOfElements, valueSize);

      //volatile uint64_t* pollPtr = dmaBuffer;
      //pollPtr += (hashTableSize/sizeof(uint64_t) * 2);
      //--pollPtr;

      volatile HashTableEntry* keyPtr = (HashTableEntry*) dmaBuffer;
      volatile HashTableEntry* entryPtr = (HashTableEntry*) (dmaBuffer + (hashTableSize/sizeof(uint64_t)));
      unsigned char* valuePtr = (unsigned char*) dmaBuffer;
      valuePtr += (hashTableSize);
      valuePtr += (numberOfQueries*sizeof(HashTableEntry));
      uint64_t entryOffset = 0;
      uint64_t valueOffset = 0;

      //write data to remote node
      communicator->put(dmaBuffer, hashTableSize, 0, 0, 0, window);

      //read all hash table entries and values
      for (uint32_t r = 0; r < numberRepetitions+1; ++r) {
         volatile HashTableEntry* ePtr = entryPtr;
         entryOffset = 0;
         ...
         memset((void*) (dmaBuffer+(hashTableSize/sizeof(uint64_t))), 0, resultSize);
         queryIndexes.clear();
         ...
         for (uint32_t q = 0; q < numberOfQueries; ++q) {
            ...
            if (!usePtrChase) {
               communicator->get((void*) ePtr, sizeof(HashTableEntry), 0, 0, entryOffset, window);
               //wait for entry
               //std::cout << "polling" << std::endl;
               while(ePtr->key == 0) {};
               //std::cout << std::dec << q << ": " << std::hex << ePtr->key << " -> " << ePtr->value_ptr << std::endl;

               uint64_t valueOffset = (uint64_t) (((uint64_t) ePtr->value_ptr) - remoteBaseAddr);
               ...
               uint8_t ptrOffset = 2;
               communicator->getValueForKeyAt((void*) vPtr, valueSize, 0, 0, entryOffset, window, keyPtr->key, mask, ptrOffset, false, 0, false);
            }
...
//std::cout << "polling for value, valuPollPtr: "<< std::hex << valuePollPtr << std::endl;
      ...
      volatile uint64_t* pollPtr = dmaBuffer;
      pollPtr += (hashTableSize/sizeof(uint64_t));
      --pollPtr;
   ...
   if (nodeId == 1) {
      ...
      unsigned char* resultPtr = (((unsigned char*) dmaBuffer) + hashTableSize);
      resultPtr += (numberOfQueries * sizeof(HashTableEntry));
      for (uint32_t q = 0; q < numberOfQueries; q++) {
         uint32_t offset = queryIndexes[q];
         unsigned char* expectedValuePtr = ((unsigned char*) dmaBuffer) + (numberOfElements * sizeof(HashTableEntry));
      ...
      memset((void*) dmaBuffer, 0, hashTableSize);
      communicator->put(dmaBuffer, hashTableSize, 0, 0, 0, window);
