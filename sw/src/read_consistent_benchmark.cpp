 ...
 * (c) 2018, ETH Zurich, Systems Group
 *
 */
...
void fillRandomData(uint64_t* block, uint32_t numberOfValues, uint32_t valueSize) {
   std::default_random_engine rand_gen(seed);
   std::uniform_int_distribution<int> distr(0, std::numeric_limits<std::uint32_t>::max());

   uint64_t* uPtr = block;
   for (uint32_t j = 0; j < numberOfValues; ++j) {
      unsigned char* currentValue = (unsigned char*) uPtr;
      for (uint32_t i = 0; i < valueSize-8; i += 8) {
         *uPtr = distr(rand_gen);
         uPtr++;
      }
      //calculate crc
      uint64_t crc = crc64(0, currentValue, valueSize-8);
      *uPtr = crc;
      uPtr++;
   }
}
...
int main(int argc, char *argv[]) {
   ...
   programDescription.add_options()("requests,r", boost::program_options::value<int>(), "Number of requests")
                                    ("elements,e", boost::program_options::value<int>(), "Number of Elements")
                                    ("valuesize,s", boost::program_options::value<int>(), "Value size in bytes")
                                    ("address", boost::program_options::value<std::string>(), "master ip address")
                                    ("warmup,w", boost::program_options::value<bool>(), "warm up")
                                    ("check,c", boost::program_options::value<bool>(), "do consistency check")
                                    ("offload,o", boost::program_options::value<bool>(), "offload consisentcy check to remote")
                                    ("failureRate,f", boost::program_options::value<int>(), "failure rate (1/f) of  consistency check");
   ...
   if (commandLineArgs.count("requests") > 0) {
      numberOfRequests = commandLineArgs["requests"].as<int>();
   }
   ...
   if (commandLineArgs.count("check") > 0) {
      doCheck = commandLineArgs["check"].as<bool>();
   }
   if (commandLineArgs.count("offload") > 0) {
      offloadCheck = commandLineArgs["offload"].as<bool>();
   }
   if (commandLineArgs.count("failureRate") > 0) {
      failureRate = commandLineArgs["failureRate"].as<int>();
   } else {
      failureRate = numberOfRequests*2;
   }

   std::cout << "Number of Requets: " << numberOfRequests << std::endl;
   std::cout << "Number of Elements: " << numberOfElements << std::endl;
   std::cout << "Value Size: " << valueSize << std::endl;

   uint64_t dataSize = numberOfElements * valueSize;
   uint64_t resultSize = numberOfRequests * valueSize;
   uint64_t allocSize = dataSize + resultSize;
   ...
   std::vector<uint32_t> requestIndexes;
   ...
   if (nodeId == 1) {
      ...
      fillRandomData(dmaBuffer, numberOfElements, valueSize);
      ...
      communicator->put(dmaBuffer, dataSize, 0, 0, 0, window);

      //read this random data multiple times
      int pkgCounter = 0;
      for (uint32_t r = 0; r < numberRepetitions+1; ++r) {
         ...
         memset((void*) (dmaBuffer+(dataSize/sizeof(uint64_t))), 0, resultSize);
         requestIndexes.clear();
         requestIndexes.reserve(numberOfRequests);

         std::this_thread::sleep_for(2s);

         for (uint32_t q = 0; q < numberOfRequests; ++q) {
            uint32_t randOffset = distr(rand_gen);
            requestIndexes.push_back(randOffset);
            valueOffset = randOffset * valueSize;

            pollPtr = (uint64_t*) vPtr;
            pollPtr += (valueSize/sizeof(uint64_t));
            pollPtr--;

            auto start = std::chrono::high_resolution_clock::now();
            if (doCheck) {
               if (offloadCheck) {
                  communicator->getConsistent((void*) vPtr, valueSize, 0, 0, valueOffset, window);
                  while (*pollPtr == 0) {};
               } else {
                  communicator->get((void*) vPtr, valueSize, 0, 0, valueOffset, window);
                  while (*pollPtr == 0) {};
                  //std::cout << "crc64 check" << std::endl;
                  uint64_t calc_crc = crc64(0, vPtr, valueSize-8);
                  pkgCounter++;
                  if (calc_crc != *pollPtr || pkgCounter == failureRate) {
                     //*pollPtr = 0;
                     memset(vPtr, 0, valueSize);
                     communicator->get((void*) vPtr, valueSize, 0, 0, valueOffset, window);
                     while (*pollPtr == 0) {};
                     //std::cout << "crc64 re-check" << std::endl;
                     calc_crc = crc64(0, vPtr, valueSize-8);
                     pkgCounter = 0;
                  }
               }
            } else {
      ...
      volatile uint64_t* pollPtr = dmaBuffer;
      pollPtr += (dataSize/sizeof(uint64_t));
      --pollPtr;

      //busy polling, waiting for random data to be written
      while (*pollPtr == 0);
   ...
   if (nodeId == 1) {
      ...
      unsigned char* resultPtr = (((unsigned char*) dmaBuffer) + dataSize);
      for (uint32_t q = 0; q < numberOfRequests; ++q) {
         uint32_t offset = requestIndexes[q];
         unsigned char* expectedValuePtr = ((unsigned char*) dmaBuffer) + (offset * valueSize);

         for (uint32_t j = 0; j < valueSize; ++j) {
            if (resultPtr[j] != expectedValuePtr[j]) {
               std::cout << std::dec << "Byte[" << j << "] in value of request " << q << " does not match, expected: ";
      ...
      memset((void*) dmaBuffer, 0, dataSize);
      communicator->put(dmaBuffer, dataSize, 0, 0, 0, window);

      //Get stats
      double totalDurationNs = 0.0;
      for (uint32_t r = 0; r < numberOfRequests; ++r) {
         totalDurationNs += durations[r];
         //std::cout << "duration[" << r << "]: " << durations[r] << std::endl;
      }
      double avgDurationNs = totalDurationNs /(double)numberOfRequests;
      double stddev = 0.0;
      for (uint32_t r = 0; r < numberOfRequests; ++r) {
         stddev += ((durations[r] - avgDurationNs) * (durations[r] - avgDurationNs));
      }
      stddev /= numberOfRequests;
      std::sort(durations.begin(), durations.end());
      double min = durations[0];
      double max = durations[numberOfRequests-1];
      double p25 = durations[(numberOfRequests/4)-1];
      double p50 = durations[(numberOfRequests/2)-1];
      double p75 = durations[((numberOfRequests*3)/4)-1];
      double p1 = 0.0;
      double p5 = 0.0;
      double p95 = 0.0;
      double p99 = 0.0;
      ...
      if (numberOfRequests >= 100) {
         p1 = durations[((numberOfRequests)/100)-1];
         p5 = durations[((numberOfRequests*5)/100)-1];
         p95 = durations[((numberOfRequests*95)/100)-1];
         p99 = durations[((numberOfRequests*99)/100)-1];
      }
      ...
      std::cout << "Size[B]: " << std::dec << dataSize << std::endl;
      ...
      std::cout << std::fixed << "#" << numberOfElements << "\t" << numberOfRequests  << "\t" << valueSize << "\t" << avgDurationNs;
      std::cout << "\t" << min << "\t" << max << "\t";
